...
public class Wall : MonoBehaviour, IDesignBehavior
{
	...
	public SerializedWall GetSerializableData()
	{
		var serWall = new SerializedWall();
		serWall.controlPoints = new List<SerializedControlPoint>();
		serWall.doors = new List<SerializedDoor>();
		foreach (var ctrlPnt in controlPoints)
			serWall.controlPoints.Add(ctrlPnt.GetSerializableData());
		foreach (var door in doors)
			serWall.doors.Add(door.GetSerializableData());
		serWall.endPointsConnected = IsLooped();
		return serWall;
	}


	/// <summary>
	/// TODO(Tristan): do some kind of check/cleaning if this is called on a pre-existing Wall.
	/// </summary>
	/// <param name="wallData"></param>
	public void CreateFromSerializedData(SerializedWall wallData)
	{
		lineRenderer.positionCount = wallData.controlPoints.Count;
		controlPoints.Capacity = wallData.controlPoints.Count;
		...
		var ctrlPntData = wallData.controlPoints[0];
		var pos = new Vector2(ctrlPntData.x, ctrlPntData.y);
		lineRenderer.SetPosition(0, pos);

		var newCtrlPnt = CreateControlPoint(lineRenderer.GetPosition(0), controlPoints.Count);
		controlPoints.Add(newCtrlPnt);

		for (int i = 1; i < wallData.controlPoints.Count; ++i)
		{
			ctrlPntData = wallData.controlPoints[i];

			pos = new Vector2(ctrlPntData.x, ctrlPntData.y);
			newCtrlPnt = CreateControlPoint(pos, i);
			lineRenderer.SetPosition(i, pos);
			controlPoints.Insert(i, newCtrlPnt);
			CreateCollider(lineRenderer.GetPosition(i), lineRenderer.GetPosition(i - 1), i - 1);
		}

		if (wallData.endPointsConnected)
			ConnectEndPoints();

		foreach (var doorData in wallData.doors)
		{
			var doorPos = new Vector2(doorData.x, doorData.y);
			var doorDObj = Instantiate(DesignManager.GetPrefab(PrefabType.DoorPrefab), doorPos, Quaternion.identity);
			var door = doorDObj.GetComponent<Door>();
			BindDoor(door, doorData.wallSegmentIndex, doorPos);
		}
	}
	...
	public WallControlPoint AddControlPointToEnd(Vector3 mouseWorldPos, int ctrlPntIndexClicked,
		ref DesignObject currentlySelectedObject, ref EditMode editMode)
	{
		...
		else if (ctrlPntIndexClicked == controlPoints.Count - 1)
		{
			...
			CreateCollider(lineRenderer.GetPosition(ctrlPntIndexClicked), mouseWorldPos, ctrlPntIndexClicked);

			++lineRenderer.positionCount;
		}
		else
			throw new Exception("This must be called only on an end control point!");
	...
	public void MovePoint(Vector3 movingPointPos, int cntrlPntIndex)
	{
		...
		if (IsLooped())
		{
			if (cntrlPntIndex == controlPoints.Count - 1)
			{
				var wallSegCollider = colliders[colliders.Count - 1];
				Vector3 stationaryPoint = controlPoints[0].transform.position;
				wallSegCollider.UpdatePosition(stationaryPoint, movingPointPos);
			}
			else if (cntrlPntIndex == 0)
			{
				var wallSegCollider = colliders[colliders.Count - 1];
				Vector3 stationaryPoint = controlPoints[controlPoints.Count - 1].transform.position;
				wallSegCollider.UpdatePosition(stationaryPoint, movingPointPos);
			}
		}
	...
	/// 
	/// </summary>
	/// <returns>List of doors that were attached to wall segment.</returns>
	...
	public Room ConvertToRoom(string roomName)
	{
		if (room != null)
			throw new Exception("Well this just ain't right");

		var roomDObj = Instantiate(DesignManager.GetPrefab(PrefabType.RoomPrefab));
		transform.SetParent(roomDObj.transform);
		room = roomDObj.GetComponent<Room>();
		var center = GetCenter();
		room.SetRoom(this, roomName, center);
		ShowControlPoints(false);

		return room;
	}

	private void _ConvertToRoom()
	{
		ConvertToRoom("New Room #");
	}
	...
	public Rect GetDimensions()
	{
		...
		var rect = new Rect();
		rect.min = new Vector2(leftmost, bottommost);
		rect.max = new Vector2(rightmost, topmost);
		return rect;
	}
	...
	private WallControlPoint AddControlPoint(Vector3 mousePos)
	{
		...
		WallControlPoint newCtrlPnt;
		List<Door> saveDoors = new List<Door>();
		if (IsLooped() && nearestPointAIndex == colliders.Count - 1)
		{
			foreach (var door in doors)
			{
				if (door.WallSegmentIndex == lineRenderer.positionCount - 1)
			...
			DesignObject newCtrlPntDO = null;
			EditMode editMode = EditMode.None;
			newCtrlPnt = AddControlPointToEnd(mousePos, controlPoints.Count - 1, ref newCtrlPntDO, ref editMode);
		...
		{
			++lineRenderer.positionCount;
			newCtrlPnt = CreateControlPoint(newPointPos, nearestPointAIndex + 1);
			++controlPoints.Capacity;
		...
		out Vector3 nearestPoint, out int nearestLineIndex)
	{
		...
		if (IsLooped())
		{
			...
			if (t > 0 && t < 1)
			{
				...
				if (dist < shortestDistance)
				{
					nearestLineIndex = lineRendererPoints.Length - 1;
					nearestPoint = nearest;
				}
			}
		}
	...
	{
		return room != null && room.isLayoutLocked;
	}
}
